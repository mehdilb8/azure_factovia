...
export default function MarrainesHero(props: MarrainesHeroProps) {
    return <div className="bg-primaryWhite text-center h-full">
        <div className="px-6 py-8">
            <img className="mx-auto rounded-full h-full w-2/5 mb-2" src={props.picture} />
            ...
            <div className="flex flex-wrap space-x-2 justify-center mb-2">
                {
                    props.tags.map((tag) => <div className="border-2 border-primary rounded-lg py-1 px-2 text-primary">{tag}</div>)
                }
            </div>
        </div>
    </div>
